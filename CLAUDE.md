...
- `crates/ledger` : notes engagées, nullifiers, Merkle (BLAKE3, prof. 16), tx, validation — testés.
  ...
  **Mempool** (phase 4, brique 3) : contrôles ordonnés du MOINS au PLUS coûteux —
  l'asymétrie de coût (~4 ms de vérification STARK pour ~105 Kio envoyés) est LE
...
- `crates/circuit` : circuit STARK **monolithe** (`monolith/`) — P1–P7 d'une tx
  ...
  1/1 ≈ 78 Kio / 1,9 ms ; 2/2 ≈ 97 Kio / 4,0 ms ; 4/4 ≈ 114 Kio / 12,3 ms (une
  `ProvedTx` 2/2 complète pèse ≈105 Kio sur le fil). ⚠️ À ±1,5 Kio près : le masquage
  tire de l'aléa FRAIS à chaque preuve — une non-régression de taille est une BANDE,
  jamais une égalité. ⚠️ Ces tailles intègrent le
...
- `crates/node` : **câblage** des briques réseau et consensus (phase 5). `message`
  ...
  (~64 o), jamais les transactions (~105 Kio) — envoyer spontanément la tx à chaque
  ...
  **Genèse explicite** : `obscura-node --genese <fichier>` (décodé par `Bloc::from_bytes`,
  ...
  `persistance::charger_ou_amorcer_etat(&genese)`. L'état GRAVE sa genèse (`VERSION_ETAT` 0x04) :
