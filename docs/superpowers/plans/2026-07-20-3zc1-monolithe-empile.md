...
**Interfaces:**
- Produces : `enum SegKind { Key, Input, Output }` ; `SEG_LEN` (lignes par segment, = longueur du plus long : le chemin de Merkle à profondeur `depth` domine → `SEG_LEN(depth)`), offsets de colonnes DANS un segment (`SPONGE_OFF=0`, `MERKLE_OFF=…`, `BALBIT_OFF=…`), largeur uniforme `SEG_WIDTH`, colonnes partagées (porteuses `OWNER_C/NK_C/ROOT_C/RHO_C[2]/CM_C[2]/LEAF_C[2]/VIN_C[2]/VOUT_C[2]`, accumulateur `S_COL`), `WIDTH` total ; `fn schedule_2in2out() -> [SegKind; 5]` = `[Key, Input, Input, Output, Output]` ; `fn seg_start(i: usize, depth) -> usize` (ligne de début du segment i) ; `fn used_rows(depth)` = `5 * SEG_LEN(depth)` ; `fn trace_len(depth) = next_pow2(used_rows(depth) + BLIND_ROWS)`.

- [ ] **Step 1 : tests géométrie** — largeur `WIDTH <= 255`, colonnes contiguës sans trou, `used_rows(depth) = 5*SEG_LEN(depth)`, `seg_start` croissant, schedule = `[Key,Input,Input,Output,Output]`, `trace_len(32)` power-of-two ≥ used+40. Écrire les asserts concrets (valeurs dérivées, pas de littéral magique).
- [ ] **Step 2 : lancer** `cargo test -p circuit monolith::layout` → FAIL puis PASS après implémentation.
- [ ] **Step 3 : implémenter** la géométrie de segment (dériver `SEG_LEN` de la longueur du chemin de Merkle `16*depth` — un segment IN doit contenir sponge commitment/leaf/nullifier ET le chemin ; vérifier que le chemin domine et fixe `SEG_LEN`). Doc de module : « segments séquentiels, couture SegKind pour 3z-c2 ».
